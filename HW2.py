...
class Course:
    ...
    def __str__(self):
        # YOUR CODE STARTS HERE
        return f"{self.cid}({self.credits}): {self.cname}" #printing the course info
...
class Loan:
    ...
    def __getloanID(self):
        loanID = random.randint(10000, 99999) #generate random id
        return loanID
...
class Person:
    ...
    def __init__(self, name, ssn):
        # YOUR CODE STARTS HERE
        self.name = name    #initiallize attributes
        self._ssn = ssn
    ...
    def __eq__(self, other):
        # YOUR CODE STARTS HERE
        if isinstance(other, Person):   #check for equality
...
class Staff(Person):
    ...
    def __init__(self, name, ssn, supervisor=None):
        ...
        super().__init__(name,ssn) #import attributes
    ...
    @property
    def id(self):
        # YOUR CODE STARTS HERE #create an id for the staff
        id = "905"
    ...
    def setSupervisor(self, new_supervisor):  #set supervisor if they are staff
        # YOUR CODE STARTS HERE
    ...
    def applyHold(self, student):
        # YOUR CODE STARTS HERE
        if isinstance(student, Student):  #putting a hold on the student
            student.holds = True
            return f"Completed!"
    ...
    def removeHold(self, student):
        # YOUR CODE STARTS HERE
        if isinstance(student, Student): #removing a hold from the student obejct
            student.holds = False
            return f"Completed!"

    def unenrollStudent(self, student):
        # YOUR CODE STARTS HERE
        if isinstance(student, Student): #unenrolling the student
            student.active = False
            return f"Completed!"

    def createStudent(self, person):
        # YOUR CODE STARTS HERE
        self.student = Student(person.name, person._ssn, "Freshman") #adding a student object
        return self.student
...
class Student(Person):
    ...
    def __createStudentAccount(self): #create an account for the student
        # YOUR CODE STARTS HERE
        if self.active == True:
    ...
    def id(self):
        # YOUR CODE STARTS HERE
        id = ""
        full_name = self.name.split()      #creating an id for the student
        for word in full_name: #getting initials
            id = id + word[:1].lower()
        id = id + self._ssn[-4:]
        return id
    ...
    def registerSemester(self):
        ...
        if (self.holds == False) and (self.active == True):
            ...
            self.semesters[sem.sem_num] = sem          #for each semester in the dictionary, create a semester object to be the valaue
    ...
    def enrollCourse(self, cid, catalog, semester):
        ...
        if self.active == False or self.holds == True:      #enroll a course in the semester chosen
        ...
        if cid in catalog.courseOfferings:
            course = self.catalog.courseOfferings[cid]
            self.semesters[semester].courses[cid] = course[0]
            credits = credits = course[0].credits
            self.account.chargeAccount(credits*self.account.CREDIT_PRICE)
            return "Course added successfully"

    def dropCourse(self, cid):
        # YOUR CODE STARTS HERE
        if (self.holds == False) and (self.active == True): #remove a course from the highest semester
            if cid in self.semesters[len(self.semesters)].courses:
                #refund half of the money
                course = self.catalog.courseOfferings[cid]
                credits = course[0].credits
                self.account.makePayment(credits*(self.account.CREDIT_PRICE/2))
                del self.semesters[len(self.semesters)].courses[cid]
    ...
    def getLoan(self, amount):
        ...
        if self.active == True:
            if self.semesters[len(self.semesters)].isFullTime == True:
                ...
                self.account.loans[self.loan.loan_id] = self.loan
