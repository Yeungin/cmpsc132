...
class AdvancedCalculator(Calculator):
    ...
    def _isVariable(self, word):
        ...
        '''
        # YOUR CODE STARTS HERE
        if word[:1].isalpha() == True and word.isalnum() == True: #check if the first character is a letter #check if the rest of the word is alphanumeric
            return True
        else:
            return False
    ...
    def _replaceVariables(self, expr):
        ...
        if self._isNumber(expr) == True: #check if the expression is only a number
            return expr
        for key in self.states:          #checking the variable names as assigned by the user
            if self._isVariable(key) == False:
                return None
        
        lst = expr.split()                       #lst of individual terms
        out = []                                 #output list
        op = ["+", "-", "*", "/", "^", "(", ")"] #list of valid characters
        for element in lst:                      #iterate through the elements in the list
            if element in self.states:           #replace the variable if valid
                out.append(str(self.states[element]))  
            elif self._isNumber(element) == True:#keeping numbers
                out.append(element)
            elif element in op:                  #keeping valid operators
                out.append(element)
            else:                                #if there is some error terminate and return none
                return None
        return ' '.join(out)                     #join the list to return the replaced string
